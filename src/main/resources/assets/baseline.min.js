...
const at=Symbol();class nt{get taskComplete(){return this.t||(1===this.i?this.t=new Promise(((t,e)=>{this.o=t,this.h=e})):3===this.i?this.t=Promise.reject(this.l):this.t=Promise.resolve(this.u)),this.t}constructor(t,e,s){this.p=0,this.i=0,(this._=t).addController(this);const i="object"==typeof e?e:{task:e,args:s};this.v=i.task,this.j=i.args,this.m=i.argsEqual??lt,this.k=i.onComplete,this.A=i.onError,this.autoRun=i.autoRun??!0,"initialValue"in i&&(this.u=i.initialValue,this.i=2,this.O=this.T?.())}hostUpdate(){!0===this.autoRun&&this.S()}hostUpdated(){"afterUpdate"===this.autoRun&&this.S()}T(){if(void 0===this.j)return;const t=this.j();if(!Array.isArray(t))throw Error("The args function must return an array");return t}async S(){const t=this.T(),e=this.O;this.O=t,t===e||void 0===t||void 0!==e&&this.m(e,t)||await this.run(t)}async run(t){let e,s;t??=this.T(),this.O=t,1===this.i?this.q?.abort():(this.t=void 0,this.o=void 0,this.h=void 0),this.i=1,"afterUpdate"===this.autoRun?queueMicrotask((()=>this._.requestUpdate())):this._.requestUpdate();const i=++this.p;this.q=new AbortController;let o=!1;try{e=await this.v(t,{signal:this.q.signal})}catch(t){o=!0,s=t}if(this.p===i){if(e===at)this.i=0;else{if(!1===o){try{this.k?.(e)}catch{}this.i=2,this.o?.(e)}else{try{this.A?.(s)}catch{}this.i=3,this.h?.(s)}this.u=e,this.l=s}this._.requestUpdate()}}abort(t){1===this.i&&this.q?.abort(t)}get value(){return this.u}get error(){return this.l}get status(){return this.i}render(t){switch(this.i){case 0:return t.initial?.();case 1:return t.pending?.();case 2:return t.complete?.(this.value);case 3:return t.error?.(this.error);default:throw Error("Unexpected status: "+this.i)}}}const lt=(t,e)=>t===e||t.length===e.length&&t.every(((t,s)=>!L(t,e[s]))),ct={limited:N`
  ...
  `};window.customElements.define("baseline-icon",class extends ot{static get styles(){return r`
      :host {
    ...
    `}static get properties(){return{featureId:{type:String},feature_id:{type:String,attribute:"feature-id"}}}updated(t){t.forEach(((t,e)=>{"feature_id"===e&&(this.featureId=this.feature_id)}))}fetchData=new nt(this,{task:async([t],{signal:e})=>{const s="https://api.webstatus.dev/v1/features/"+t,i=await fetch(s,{signal:e,cache:"force-cache"});if(!i.ok)throw new Error(i.status);return i.json()},args:()=>[this.featureId]});renderSupportIcon(t,e){const s="limited"===t?e?.status||"unavailable":t;return N`<browser-support-icon class="support-${s}">
      ${ut["newly"===s||"widely"===s?"available":s]}
    </browser-support-icon>`}getBaselineDate(t){return t.baseline.low_date?new Intl.DateTimeFormat("en-US",{year:"numeric",month:"long"}).format(new Date(t.baseline.low_date)):""}getDescriptionDate(t,e){return"newly"===t&&e?`Since ${e} this feature works across the latest\n        devices and browser versions. This feature might not work in older\n        devices or browsers.`:"widely"===t&&e?`This feature is well established and works across many\n        devices and browser versions. It’s been available across browsers\n        since ${e}`:Ct[t].defaultDescription}renderTemplate(t,e){const s=t.baseline.status||"no_data",i="limited"===s||"no_data"===s?"":N`<strong>Baseline</strong> `,o=e?"Loading...":Ct[s].title,r="newly"===s?N`<span class="baseline-badge">newly available</span>`:"",a=this.getBaselineDate(t),n=this.getDescriptionDate(s,a),l="newly"===s&&a?a.split(" ")[1]:"",{chrome:c,edge:p,firefox:h,safari:d}=t.browser_implementations||{};return N` <div class="name">${t.name}</div>
      <details>
        <summary
          aria-label="${((t,e,s,i="no",o="no",r="no",a="no")=>("Unknown availability"===t&&(i=o=r=a="unknown"),`Baseline: ${t}${e?` ${e}`:""}${s?" (newly available)":""}. Supported in Chrome: ${"available"===i?"yes":i}. Supported in Edge: ${"available"===o?"yes":o}. Supported in Firefox: ${"available"===r?"yes":r}. Supported in Safari: ${"available"===a?"yes":a}.`))(o,l,r,c?.status,p?.status,h?.status,d?.status)}"
        >
          ...
          <div class="baseline-status-title" aria-hidden="true">
            ...
            <div class="baseline-status-browsers">
              <span>${pt} ${this.renderSupportIcon(s,c)}</span>
              <span>${ht} ${this.renderSupportIcon(s,p)}</span>
              <span>${dt} ${this.renderSupportIcon(s,h)}</span>
              <span>${ft} ${this.renderSupportIcon(s,d)}</span>
            </div>
          </div>
        </div>
      ...
      <p>
        ${"no_data"===s?"":N`<a href="https://github.com/web-platform-dx/web-features/blob/main/features/${t.feature_id}.yml" target="_top">Learn more</a>`}
      </p>
    </details>`}render(){const t={baseline:{status:"no_data"},name:this.featureId||"Unknown feature"};return this.featureId?this.fetchData.render({pending:()=>this.renderTemplate(t,!0),complete:e=>e&&e.baseline?this.renderTemplate(e):this.renderTemplate(t),error:()=>this.renderTemplate(t)}):this.renderTemplate(t)}})}();
